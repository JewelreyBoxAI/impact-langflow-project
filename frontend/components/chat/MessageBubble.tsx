...
export function MessageBubble({ message, className }: MessageBubbleProps) {
  ...
  return (
    <div className={cn(
      ...
      {/* Message Content */}
      <div className={cn(
        "flex flex-col gap-1 max-w-[85%] sm:max-w-[75%] md:max-w-[70%] min-w-0",
        isUser && "items-end",
        ...
        {!isUser && !isSystem && (
          <div className="text-xs text-muted-foreground font-medium">
            {message.senderName || 'Recruiting Agent'}
          </div>
        )}

        {/* Message Bubble */}
        <div className={cn(
          "rounded-lg px-3 py-2 text-sm break-words hyphens-auto overflow-wrap-anywhere",
